...
Feature: Pyspark Pipeline Step -> Asynchronous Inbound/Outbound Type Patterns
  ...
  Scenario Outline: Void Inbound (with Non-Messaging Outbound)
    When an asynchronous step with void inbound type and <outboundType> outbound type is defined
    Then the step execution is async
    And the step execution has no inbound parameter
    Examples:
      | outboundType |
      | void         |
      | native       |
  ...
  Scenario Outline: Void Outbound (with Non-Messaging Inbound)
    When an asynchronous step with <inboundType> inbound type and void outbound type is defined
    Then the step execution is async
    And the step execution has no outbound return type
      | inboundType |
      | void        |
      | native      |

  Scenario Outline: Native Inbound (with Non-Messaging Outbound)
    When an asynchronous step with native inbound type and <outboundType> outbound type is defined
    Then the step execution is async
    And the step execution has an inbound parameter of type "DataFrame"
    Examples:
      | outboundType |
      | void         |
      | native       |
  ...
  Scenario Outline: Native Outbound (with Non-Messaging Inbound)
    When an asynchronous step with <inboundType> inbound type and native outbound type is defined
    Then the step execution is async
    And the step execution has an outbound return type of "DataFrame"
    Examples:
      | inboundType |
      | void        |
      | native      |
  ...
  Scenario Outline: Messaging Inbound
    When an asynchronous step with messaging inbound type and <outboundType> outbound type is defined
    Then the step execution is async
  ...
  Scenario Outline: Messaging Outbound
    When an asynchronous step with <inboundType> inbound type and messaging outbound type is defined
    Then the step execution is async

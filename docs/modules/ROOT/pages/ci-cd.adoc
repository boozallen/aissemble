...
tool that can deploy your entire project quickly. The aiSSEMBLE team encourages the usage of
https://helmfile.readthedocs.io/en/latest/[Helmfile,role=external,window=_blank] as your local CD for Kubernetes.
Helmfile is a ultra-lightweight solution that introduces minimal maintenance overhead, acting as a thin wrapper around
Helm deployments. By default, aiSSEMBLE will generate Helmfile deployment files to get you started.  Helmfile can deploy
your project (in its entirety or partially) with a single command.
...
aiSSEMBLE enables standardized delivery and monitoring to drive consistency and reliability. Helmfile is a also CD tool
which deploys and continuously monitors running applications and compares the current, live state against the desired target
state.

...
| `relations` (xref:#_record_relation_options[details])
| No
| None
| A list of relations from your record to other record types.
...
[#_record_relation_options]
=== Record Relation Options
The following options are available on the `relations` record element:
...
| Yes
| None
| The name of the relation that is being modeled.

| `relations/relation/package`
| Yes
| None
| Provides a namespace that will be leveraged to differentiate between instances and provide language-specific
namespacing (e.g., package in Java, namespace in XSD).
